...
const scramblesMatcher = () => {
  try {
    ...
    const backupRoundData = backupSheet
      .getRange(1, 2, backupSheet.getLastRow(), 2)
      .getValues() as [SupportedEventId, number][];
    getEventIds().forEach((eventId) => {
      const dbRoundId = backupRoundData.find((row) => row[0] === eventId)![1];
      const scrambleSets = (
      ...
      const competitorRows = competitorsSheet
        .getRange(2, 1, nCompetitors, 2)
        .getValues();
      ...
      resultRows.forEach((row, rowIndex) => {
        ...
        const lastMatchNumber = resultRows
          ...
          .at(-1)![0];
        switch (true) {
          case lastMatchNumber === nCompetitors:
            ranking =
              getSetWinners(eventId, resultRows)[0][2] === row[2] ? 1 : 2;
            break;
          case lastMatchNumber === nCompetitors - 1:
            ranking =
              getSetWinners(
                eventId,
                resultRows.filter((resultRow) => resultRow[0] < nCompetitors),
              )[0][2] === row[2]
                ? 3
                : 4;
            break;
          case lastMatchNumber >= nCompetitors - 7 &&
            lastMatchNumber <= nCompetitors - 4:
            const bests8 = competitorRows
              .filter((competitorRow) => {
                const lastMatchNumber = resultRows
                  ...
                  .at(-1)![0];
                return (
                  lastMatchNumber >= nCompetitors - 7 &&
                  lastMatchNumber <= nCompetitors - 4
                );
              })
              .map(
            ...
            break;
          default:
            const bests12or16 = competitorRows
              .filter(
                (competitorRow) =>
                  resultRows
                    .filter((resultRow) => resultRow[2] === competitorRow[0])
            ...
            ranking =
              bests12or16.filter(
                (r) => r[1] < bests12or16.find((r) => r[0] === row[2])![1],
              ).length + 9;
            break;
        }
    ...
    folder.createFile(
      `h2h_results_${getCompetitionId(ss)}.csv`,
...
const setCompetitors = (eventId: SupportedEventId) => {
  try {
    ...
    const nCompetitors = competitorsSheet.getLastRow();
    const wcif = getWcif();
    const competitorRows = wcif.events
      .find((event) => event.id === eventId)!
      .rounds.at(-2)!
      .results.filter(
        (result) => result.ranking && result.ranking <= nCompetitors,
      )
      ...
      .map((result) => {
        ...
        return [person.registrantId, person.name];
      });
    competitorsSheet
      .getRange(2, 1, competitorRows.length, 2)
...
const setCompetitors333bf = () => setCompetitors("333bf");
const setCompetitors333oh = () => setCompetitors("333oh");
      ...
      if (matchSetIndexes.length > 0) {
        ...
        if (
          ...
          > = {
            4: {
              1: [
                [3, 0, loserRegistrantId],
                [4, 0, winnerRegistrantId],
              ],
              2: [
                [3, 1, loserRegistrantId],
                [4, 1, winnerRegistrantId],
              ],
            },
            8: {
            ...
            12: {
              1: [[5, 1, winnerRegistrantId]],
              2: [[6, 0, winnerRegistrantId]],
              3: [[7, 1, winnerRegistrantId]],
              4: [[8, 0, winnerRegistrantId]],
              5: [[9, 0, winnerRegistrantId]],
              6: [[9, 1, winnerRegistrantId]],
              7: [[10, 0, winnerRegistrantId]],
              8: [[10, 1, winnerRegistrantId]],
              9: [
                [11, 0, loserRegistrantId],
                [12, 0, winnerRegistrantId],
              ],
              10: [
                [11, 1, loserRegistrantId],
                [12, 1, winnerRegistrantId],
              ],
            },
            16: {
          ...
          const matchesAndIndexesAndIds =
            MATCHES_AND_INDEXES_AND_IDS[
              ss.getSheetByName(`${eventId} Competitors`)!.getLastRow() - 1
            ][matchDetails[0]];
...
const submitResults333bf = () => submitResults("333bf");
const submitResults333oh = () => submitResults("333oh");
