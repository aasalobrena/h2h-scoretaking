const setCompetitors = () => {
  try {
    ...
    const competitorRows = wcif.events
      ...
      .rounds.at(-2)!
      .results.filter(
        (result) => result.ranking && result.ranking <= N_COMPETITORS,
      )
...
const downloadCSV = (jsonString: string) => {
  try {
    const scrambleSets = (JSON.parse(jsonString).wcif as Competition).events
      .find((event) => event.id === EVENT_ID)!
      .rounds.at(-1)!.scrambleSets!;
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const competitorRows = ss
      .getSheetByName("Competitors")!
      .getRange(2, 1, N_COMPETITORS, 3)
      .getValues();
    ...
    resultRows.forEach((row, rowIndex) => {
      let ranking = 0;
      switch (
        resultRows.filter((resultRow) => resultRow[2] === row[2]).at(-1)![0]
      ) {
        case N_COMPETITORS:
          ranking = getSetWinners(resultRows)[0][2] === row[2] ? 1 : 2;
          break;
        case N_COMPETITORS - 1:
          ranking =
            getSetWinners(
              resultRows.filter((resultRow) => resultRow[0] < N_COMPETITORS),
            )[0][2] === row[2]
              ? 3
              : 4;
          break;
        ...
        case 4:
          ...
          ranking =
            bests12.filter(
              (r) => r[1] < bests12.find((r) => r[0] === row[2])![1],
            ).length + 9;
          break;
        case 5:
        case 6:
        ...
        case 8:
          ...
          ranking =
            bests8.filter((r) => r[1] < bests8.find((r) => r[0] === row[2])![1])
              .length + 5;
          break;
        ///////////////////////////////////////////////////
      }
    ...
    template.competitionId = COMPETITION_ID;
    template.csv = values.map((r) => r.join(",")).join("\n");
    SpreadsheetApp.getUi().showModalDialog(
      template.evaluate().setWidth(10).setHeight(10),
      "Downloading...",
    );
  } catch (err) {
    showError(err as string);
...
const scramblesMatcher = () =>
  SpreadsheetApp.getUi().showModalDialog(
    HtmlService.createHtmlOutputFromFile("upload").setWidth(400).setHeight(200),
...
const submitResults = () => {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const scoretakingSheet = ss.getSheetByName("Scoretaking")!;
    const resultsSheet = ss.getSheetByName("Results")!;
    ...
    if (results[0][0] && results[1][0]) {
      ...
      if (matchSetIndexes.length > 0) {
        const winnersRows = getSetWinners(resultRows.slice(0, rowIndex + 2));
        if (
          ...
          const matchesAndIndexesAndIds =
            MATCHES_AND_INDEXES_AND_IDS[matchDetails[0]];
    ...
    for (let i = 1; i < resultRows.length; i += 2) {
      const winnersRows = getSetWinners(resultRows.slice(0, i + 1));
